...
Sample_Name <- "sample_name" #name of dataset
...
coph <- cophenetic(match$phy) #creates cophenetic distance matrix relating species
...
for(i in range){ #in this iteration, data is structured with peaks as rows and samples as columns - returns distances between samples
  bMNTD.rand = as.matrix(comdistnt(t(match$data), taxaShuffle(coph), abundance.weighted = F, exclude.conspecifics = F)) #calculates bMNTD relating samples with randomized distance matrix
